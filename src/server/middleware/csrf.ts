...
export const csrfProtect: MiddlewareHandler = async (c, next) => {
  ...
  const selfOrigins = new Set([config.appOrigin, `http://${host}`, `https://${host}`]);

  const origin = c.req.header("origin");
  if (origin && origin !== "null") {
    if (!selfOrigins.has(origin)) return c.json({ error: "cross-origin request rejected" }, 403);
    return next();
  }
  ...
  if (!selfOrigins.has(refererOrigin)) return c.json({ error: "cross-origin request rejected" }, 403);
  await next();
};
